...
const MOCK_DATA = {
  proyectos: [{ nombre: 'Demo Project', tipo: 'Proyecto', estado: 'Activo' }],
  requisitos: [
    {
      id: '1',
    ...
    try {
      ...
      if (parsed.proyectos) {
        parsed.proyectos = parsed.proyectos.map(p => {
          const obj = typeof p === 'string' ? { nombre: p, tipo: 'Proyecto' } : p
          return { estado: 'Activo', ...obj }
        })
      }
...
function reducer(state, action) {
  switch (action.type) {
    case 'ADD_PROJECT':
      return { ...state, proyectos: [...state.proyectos, { estado: 'Activo', ...action.payload }] }
    case 'UPDATE_PROJECT': {
      const { oldName, newName, tipo, estado } = action.payload
      return {
        ...state,
        proyectos: state.proyectos.map(p => p.nombre === oldName ? { ...p, nombre: newName, tipo, estado: estado || p.estado || 'Activo' } : p),
    ...
    case 'UPDATE_PROJECT_ESTADO':
      return {
        ...state,
        proyectos: state.proyectos.map(p => p.nombre === action.payload.nombre ? { ...p, estado: action.payload.estado } : p)
      }
    case 'DELETE_PROJECT':
      return {
        ...state,
...
export const PROYECTO_ESTADOS = ['Activo', 'En pausa', 'Completado']
export const PROYECTO_ESTADO_CONFIG = {
  'Activo':     { color: 'text-emerald-400', bg: 'bg-emerald-500/15', border: 'border-emerald-500/30', dot: 'bg-emerald-400' },
  'En pausa':   { color: 'text-amber-400',   bg: 'bg-amber-500/15',   border: 'border-amber-500/30',   dot: 'bg-amber-400' },
  'Completado': { color: 'text-cyan-400',    bg: 'bg-cyan-500/15',    border: 'border-cyan-500/30',    dot: 'bg-cyan-400' },
}
