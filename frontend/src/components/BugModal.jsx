...
export default function BugModal({ onClose, editBug }) {
  ...
  const [proyectoId, setProyectoId] = useState(editBug ? editBug.proyectoId : state.proyectos[0]?.id || '')
  ...
  function handleSubmit(e) {
    ...
    if (editBug) {
      dispatch({ type: 'UPDATE_BUG', payload: { id: editBug.id, ...bugData } })
    } else {
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select value={proyectoId} onChange={e => setProyectoId(e.target.value)} className={inputCls}>
              {state.proyectos.map(p => <option key={p.id} value={p.id} className="bg-surface-700">{p.nombre}</option>)}
            </select>
          </div>
