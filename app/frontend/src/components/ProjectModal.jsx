...
export default function ProjectModal({ onClose, editProject }) {
  ...
  function handleSubmit(e) {
    ...
    const existe = state.proyectos.some(p => p.nombre.toLowerCase() === trimmedName.toLowerCase() && (!editProject || p.id !== editProject.id))
    if (existe) {
    ...
    if (editProject) {
      dispatch({ type: 'UPDATE_PROJECT', payload: { id: editProject.id, nombre: trimmedName, tipo } })
    } else {
