...
import { useApp, getProjectColor, getProyectoNombre } from '../context/AppContext'
...
export default function BugsTracker() {
  ...
  const groupedActiveBugs = activeBugs.reduce((acc, b) => {
    const nombre = getProyectoNombre(state, b.proyectoId)
  ...
  const BugCard = ({ bug }) => {
    const proyectoNombre = getProyectoNombre(state, bug.proyectoId)
    const pColor = getProjectColor(proyectoNombre, state.proyectos)
